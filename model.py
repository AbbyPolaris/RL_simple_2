...
def Q_learning(env,
    ...
    init_alpha=1,
    min_alpha=0.1,
    alpha_decay_ratio=0.06,
    init_epsilon=1,
    min_epsilon=.1,
    epsilon_decay_ratio=0.06,
    n_episodes=30000 , dtype=np.float64):
    
    nS = 2**(env.observable_size+6)#3**(2*len(env.observable_space) +1) // we should correct this, we need to make observable space.
    nA = len(action_space)
